...
module.exports = function (opts) {
    ...
    router.post('/notifyRegTx', function (req, res) {
        ...
        config.web3.eth.getTransaction(tx_id, function (err, tx_details) {
            if (err) {
                logger.log(prelog + 'error getting tx_details from blockchain: ' + err);
                return res.json({ ok: false, err: 'error getting tx details from blockchain' });
            }
            if (!tx_details) {
                logger.log(prelog + 'no details for tx with this hash: ' + tx_id);
                return res.json({ ok: false, err: 'no details for tx with this hash: ' + tx_id });
            }
            ...
            if (tx_details.to !== config.cconf.address) {
                ...
                return res.json({ ok: false, err: 'to-address in transaction does not match contract address' });
            }
            ...
            if (tx_details.from !== wallet) {
                ...
                return res.json({ ok: false, err: 'from-address in transaction does not match user wallet' });
            }
            ...
            config.contract.user_address_by_creation_block(wallet, tx_bn, function (err, addr_index) {
                if (err) {
                    logger.log(prelog + 'error getting address by tx_bn: ' + err);
                    return res.json({ ok: false, err: 'error getting address from transaction' });
                }
                logger.log(prelog + 'addr_index: ' + JSON.stringify(addr_index));

                if (!addr_index[0]) {
                    logger.log(prelog + 'address not found by creation block number');
                    return res.json({ ok: false, err: 'address not found by creation block number' });
                }
                ...
                config.contract.user_address(wallet, addr_index[1], function (err, adet1) {
                    if (err) {
                        logger.log(prelog + 'error getting address details from contract: ' + err);
                        return res.json({ ok: false, err: 'error getting address details from contract' });
                    }
                    ...
                    config.contract.user_address_info(wallet, addr_index[1], function (err, adet2) {
                        if (err) {
                            logger.log(prelog + 'error getting more address details from contract: ' + err);
                            return res.json({ ok: false, err: 'error getting additional address details from contract' });
                        }
                        ...
                        post_api.create_postcard(wallet, addr, tx_id, info.confirmation_code_plain, function (err, result) {
                            if (err) {
                                logger.log(prelog + 'error from create_postcard: ' + err);
                                return res.json({ ok: false, err: 'error while sending postcard' });
                            }
